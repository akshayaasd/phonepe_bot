    ...
    try:
        ...
        if count!="":
            if count>0:
                ...
                # is_inserted = mongo_db.mongo_insert( "input", request_data)
                # if is_inserted:
        ...
        no_answer_seq = input_collection.get('no_answer_seq', 0)
        triggered_from = input_collection.get('triggered_from', "")
        call_sequence_mapping = input_collection.get('call_sequence_mapping', {})
        inserted_on = input_collection.get('inserted_on', None)

        full_name = str(input_collection.get("full_name", "Customer"))
        agent_name = "alishba"
        ...
        allocation_month = str(input_collection.get("allocation_month", "2025-09-04"))
        loan_id = str(input_collection.get("loan_id", "1234"))
        company_id = str(input_collection.get("company_id", "a3e09e17-4a67-4fdf-8225-7b2cd6299e02"))
        communication_level = str(input_collection.get("communication_level", "customer"))
        company_name = "PhonePe"
        product_description = str(input_collection.get("product_description", ""))
        bounce_charges = str(input_collection.get("bounce_charges", 0))
        registration_numbers = str(input_collection.get("registration_number", ""))
        registration_number = str(registration_numbers)
        interest_rate = str(input_collection.get("interest_rate", ""))
        principal_amount = str(input_collection.get("principal_amount", 0))
        loan_tenure = str(input_collection.get("loan_tenure", ""))
        phone_number_input_date = str(input_collection.get("phone_number_input_date", ""))

        last_4_digits = loan_id[-4:]

        

        due_amount_hin= num2words(due_amount, lang='hi')
        due_amount_eng= num2words(due_amount, lang='en')

        bounce_charges_eng = num2words(bounce_charges, lang='hi')
        bounce_charges_hin = num2words(bounce_charges, lang='en')

        principal_amount_eng = num2words(principal_amount, lang='hi')
        principal_amount_hin = num2words(principal_amount, lang='en')
        
        due_date_eng = str(due_date_format(due_date,"english"))
        due_date_hin = str(date_audio(due_date))
        
        
        today = date.today()
        todays_date = today.strftime("%Y-%m-%d")
        due_days = calculate_due_date_difference(due_date, todays_date)
        try:
            parsed_date = datetime.fromisoformat(inserted_on)
            inserted_on_val =  f"ISODate({parsed_date.isoformat()})"
        except Exception as e:
            print(f"we have exception in inserted_on :: {e} - {sender_id}")
        
        input_date = input_collection.get("input_date", "")
        if language == "english":
            initial_msg = f"Hello {full_name}, I'm alishba calling from PhonePe about your pending EMI of {due_amount_eng} rupees due on {due_date_eng}. Can you make this payment today?"
        else:
            initial_msg = f"Hello {full_name}, मैं alishba बोल रही हूँ PhonePe से, आपके pending EMI के बारे में जो {due_date_hin} तक {due_amount_hin} रुपये का due है। क्या आप यह payment आज कर सकते हैं?"
        ...
        user_context = {
            ...
            "answered_seq": answered_seq,
            "no_answer_seq": no_answer_seq,
            "phone_number": phone_number,
            "call_sequence_mapping": call_sequence_mapping,
            ...
            "phone_number_input_date": phone_number_input_date,
            "due_days": due_days,
        ...
        logger.exception("Exception in initial message fetch and udpate: " + str(e))
        response_json_inya = {}  
    ...
    try:
        ...
        if empty_data == True or empty_conv_data == True:
            ...
            try:
                ...
                no_answer_seq = bot_data.get("user_context",{}).get("no_answer_seq",0)
                ...
                inserted_on = bot_data.get("user_context",{}).get("inserted_on")
                full_name = bot_data.get("user_context",{}).get("full_name","")
                due_date = bot_data.get("user_context",{}).get("due_date","")
                ...
                overdue_amount = bot_data.get("user_context",{}).get("overdue_amount","")
                mention_date = bot_data.get("user_context",{}).get("mention_date","")
                company_name = bot_data.get("user_context",{}).get("company_name","")
                email_id = bot_data.get("user_context",{}).get("email_id","")
                product_description = bot_data.get("user_context",{}).get("product_description","")
                phone_number_input_date = bot_data.get("user_context",{}).get("phone_number_input_date","")

                due_date_eng = bot_data.get("user_context", {}).get("due_date_eng", "")
                due_date_hin = bot_data.get("user_context", {}).get("due_date_hin", "")
                due_amount_eng = bot_data.get("user_context", {}).get("due_amount_eng", "")
                due_amount_hin = bot_data.get("user_context", {}).get("due_amount_hin", "")
                todays_date = bot_data.get("user_context", {}).get("todays_date", "")
                interest_rate = bot_data.get("user_context", {}).get("interest_rate", "")
                loan_tenure = bot_data.get("user_context", {}).get("loan_tenure", "")
                registration_number = bot_data.get("user_context", {}).get("registration_number", "")
                bounce_charges = bot_data.get("user_context", {}).get("bounce_charges", "")
                bounce_charges_eng = bot_data.get("user_context", {}).get("bounce_charges_eng", "")
                bounce_charges_hin = bot_data.get("user_context", {}).get("bounce_charges_hin", "")
                principal_amount = bot_data.get("user_context", {}).get("principal_amount", "")
                principal_amount_hin = bot_data.get("user_context", {}).get("principal_amount_hin", "")
                principal_amount_eng = bot_data.get("user_context", {}).get("principal_amount_eng", "")
                due_days = bot_data.get("user_context", {}).get("due_days", "")
                last_triggered_date = bot_data.get("user_context", {}).get("last_triggered_date", "")
                allocation_month = bot_data.get("user_context", {}).get("allocation_month", "")
                loan_id = bot_data.get("user_context", {}).get("loan_id", "")
                company_id = bot_data.get("user_context", {}).get("company_id", "")
                registration_numbers = bot_data.get("user_context", {}).get("registration_numbers", "")
                last_4_digits = bot_data.get("user_context", {}).get("last_4_digits", "")
                communication_level = bot_data.get("user_context", {}).get("communication_level", "")
                inbound_bot_id = bot_data.get("user_context", {}).get("inbound_bot_id", "")
            ...
            bot_data['no_answer_seq'] = no_answer_seq
            bot_data['call_sequence_mapping'] = call_sequence_mapping
            bot_data['input_date'] = input_date
            bot_data['inserted_on'] = inserted_on
            bot_data['full_name'] = full_name
            bot_data['language'] = language
            bot_data['due_date'] = due_date
            bot_data['agent_name'] = agent_name
            bot_data['lender_name'] = lender_name
            bot_data['due_amount'] = due_amount
            bot_data['mention_date'] = mention_date
            bot_data['overdue_amount'] = overdue_amount
            bot_data['company_name'] = company_name
            bot_data['email_id'] = email_id
            bot_data['product_description'] = product_description
            bot_data['due_date_eng'] = due_date_eng
            bot_data['due_date_hin'] = due_date_hin
            bot_data['due_amount_eng'] = due_amount_eng
            bot_data['due_amount_hin'] = due_amount_hin
            bot_data['todays_date'] = todays_date
            bot_data['interest_rate'] = interest_rate
            bot_data['loan_tenure'] = loan_tenure
            bot_data['registration_number'] = registration_number
            bot_data['bounce_charges'] = bounce_charges
            bot_data['bounce_charges_eng'] = bounce_charges_eng
            bot_data['bounce_charges_hin'] = bounce_charges_hin
            bot_data['principal_amount'] = principal_amount
            bot_data['principal_amount_hin'] = principal_amount_hin
            bot_data['principal_amount_eng'] = principal_amount_eng
            bot_data['due_days'] = due_days
            bot_data['last_triggered_date'] = last_triggered_date
            bot_data['allocation_month'] = allocation_month
            bot_data['loan_id'] = loan_id
            bot_data['company_id'] = company_id
            bot_data['registration_numbers'] = registration_numbers
            bot_data['last_4_digits'] = last_4_digits
            bot_data['communication_level'] = communication_level
            bot_data['inbound_bot_id'] = inbound_bot_id
            bot_data['phone_number_input_date'] = phone_number_input_date
